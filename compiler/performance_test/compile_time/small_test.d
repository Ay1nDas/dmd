module dmd.compiler.performance_test.compile_time.small_test;
...
void main()
{
    ...
    enum fib = computeCTFE(35); // Very slow at compile time
    writeln("CTFE Fib(35): ", fib);

    enum val = RecursiveTemplate!(50); // Deep template recursion
    writeln("RecursiveTemplate(1500): ", val);

    writeln("DeepStaticIf(1000): ", DeepStaticIf!(50));
    writeln("HugeEnum: ", HugeEnum);
    writeln("Trait Check: ", traitsResult, " | ", traitsResult2);
    writeln("NestedTemplate: ", NestedTemplate!(5));
}
