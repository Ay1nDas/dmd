module dmd.compiler.performance_test.compile_time.medium_test_2;
...
void main()
{
        enum fib = computeCTFE(36); // Very slow at compile time
        writeln("CTFE Fib(35): ", fib);

        enum val = RecursiveTemplate!(500); // Deep template recursion
        writeln("RecursiveTemplate(500): ", val);

        writeln("DeepStaticIf(500): ", DeepStaticIf!(500));
        writeln("HugeEnum: ", HugeEnum);
        writeln("Trait Check: ", traitsResult, " | ", traitsResult2);
        writeln("NestedTemplate: ", NestedTemplate!(500));
}
